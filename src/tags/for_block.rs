...
fn parse_attr(arguments: &mut Iterator<Item = TagToken>) -> Result<Expression> {
    if arguments.next().unwrap_or_else(|| panic!("Errors not implemented. Token expected.")).as_str() != ":" {
        panic!("Errors not implemented. Unexpected token.");
    }

    arguments.next().unwrap_or_else(|| panic!("Errors not implemented. Token expected.")).expect_value()
}
...
pub fn for_block(
    ...
    let var_name = arguments.next().unwrap_or_else(|| panic!("Errors not implemented. Token expected."));
    let var_name = var_name.expect_identifier()?.to_string();

    if arguments.next().unwrap_or_else(|| panic!("Errors not implemented. Token expected.")).as_str() != "in" {
        panic!("Errors not implemented. Unexpected token.");
    }

    let range = arguments.next().unwrap_or_else(|| panic!("Errors not implemented. Token expected."));
    let range = if let Ok(array) = range.expect_value() {
        Range::Array(array)
    } else if let Ok((start, stop)) = range.expect_range() { // Ranges will panic for now because error handling is not implemented
        Range::Counted(start, stop)
    } else {
        panic!("Errors not implemented. Expected Array or Range.");
    };
...
pub fn tablerow_block(
    ...
    let range = if let Ok(array) = range.expect_value() {
        Range::Array(array)
    } else if let Ok((start, stop)) = range.expect_range() {
        Range::Counted(start, stop)
    } else {
        panic!("Errors not implemented. Expected Array or Range.");
    };
